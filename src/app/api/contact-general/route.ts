...
export async function POST(req: NextRequest): Promise<NextResponse | Response> {
  try {
    ...
    const { name, email, whatsapp_number, topic, message, recaptchaToken } =
    ...
    const strapiResponse = await fetch(
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/contact-form-generals`,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          data: { name, email, whatsapp_number, topic, message },
        }),
      }
    );
    ...
    const emailResponse = await fetch(
      `${process.env.NEXT_PUBLIC_SITE_URL}/api/send-email-contact-general`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, name, whatsapp_number, topic, message }),
      }
    );
