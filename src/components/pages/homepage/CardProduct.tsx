...
export default function CardProduct({
  ...
  useGSAP(() => {
    ...
    if (!textRef.current || !cardRef.current || !imageRef.current) return;
    ...
    // Initial state - text is hidden (moved down and clipped)
    gsap.set(elements, {
  ...
  return (
    <div
      ref={cardRef}
      className={clsx(
        "w-full h-[60vh] overflow-hidden rounded-32d relative cursor-pointer",
        className
      )}
    >
      <h3
        ref={textRef}
        className="absolute z-20 bottom-[10%] text-ruby-red font-span font-bold left-1/2 -translate-x-1/2 text-center"
      >
        {title}
      </h3>
